...
<% 
   ...
   If UCase(Request.ServerVariables("HTTPS")) = "ON" Then
    ...
    If UCASE(SITE_URL &"test/" & CurrentFilename) <> UCASE(CurrentURL) Then
        if Request.ServerVariables("QUERY_STRING")  & "" <> "" then
            CurrentFilename  = CurrentFilename & "?"&  Request.ServerVariables("QUERY_STRING")
        end if
        Response.Redirect(SITE_URL &"test/" & CurrentFilename)
    End If
...
<head>
  ...
  <meta name="author" content="">
    ...
    <link href="../css/product.css?v=1.6" rel="stylesheet">
  ...
  <script>
   ...
   var individualpostcodeschecking ;
    ...
    <script type="text/javascript" src="../scripts/fancybox/jquery.fancybox.pack.js?v=2.1.5"></script>	
        <% if individualpostcodeschecking = 0 then %>  
    <script src="../scripts/Locationpicker.js?v=1.2"></script>
    <% end if %>
			...
			<div style="width:165px; height : 450px; overflow : auto; " class="hidden-xs hidden-sm"><ul class="nav nav-stacked nav-pills navdesktop" style="width:155px;overflow : auto;    height: 80vh;">
                ...
                objRdsMainCategory.Open "SELECT DISTINCT mc.ID,mc.Name,mc.Description,displayorder FROM MenuCategories AS mc INNER JOIN MenuItems AS mi ON mc.Id = mi.IdMenuCategory WHERE  mc.IdBusinessDetail=" & vRestaurantId & " and (((mi.idbusinessdetail)=" & vRestaurantId & ")) and mi.hidedish<>-1 ORDER BY mc.displayorder;", objCon
                        if not objRdsMainCategory.EOF then
                            ...
                            %>
                            <li ><a href="#menucat_<%=objRdsMainCategory("ID") %>" class="catlink" onclick="$('#categroup-<%=objRdsMainCategory("ID")%>').hide();$('#categroup-<%=objRdsMainCategory("ID")%>').slideDown('show');">
...
<!-- End update menu bar -->
                ...
                while not objRdsMainCategory.EOF
                        ...
                        %>
                            ...
                            <div class="product-line-heading clearfix">
                            <h4 class="product-line-heading__cat pull-left" onclick="ShowdishpropertiesV2('categroup-<%=categoryID %>')">
                            <a id="menucat_<%=categoryID %>" name="menucat_<%=categoryID %>" ></a>
                            <%= CategoryName%>   
                            </h4>
                            <div class="product-line-heading__icon-wrapper is-vertical-center" onclick="ShowdishpropertiesV2('categroup-<%=categoryID %>')">
                        ...
                        <%
                         ' Load Menu Item 
                            ...
                            while not objRds_MenuItem.EOF
                                   ...
                                   mipallowtoppings = objRds_MenuItem("miallowtoppings")
                                        ...
                                        <% 
								            ...
								            If Photo <> "" Then 
								               ...
								               photo=1%>
                                                   <div  class="product10w photo" data-toggle="modal" data-target="#lightbox">  
                                                        <img src="Images/<%=vRestaurantId %>/<%= objRds_MenuItem("Photo")%>" class="img-rounded" alt="<%= MenuItemName%>" style="vertical-align: top;width:98%;max-width:40px;" /> 
                                                            <div class="overlay">
                                            ...
                                            <% If Code <> "" Then 
								                code=1%>
                                                   ...
                                                   <div class="product-line__number" <%=styleMarginleft %>> <%= objRds_MenuItem("Code")  %>.</div>
                                                <%End If %>
								...
								dishtoppingstext=""
								if miallowtoppings & "" <> "0" or mipallowtoppings & "" <> "0" then
                                        
                                        Set objRds_toppings = Server.CreateObject("ADODB.Recordset")           
                                            SQL = "SELECT * FROM MenuToppings where  IdBusinessDetail=" & vRestaurantId
                                        if miallowtoppings & "" <> "0" and mipallowtoppings & "" = "0" then
                                            SQL =SQL & " and toppinggroupid=" & miallowtoppings
                                        elseif miallowtoppings = 0 and mipallowtoppings <> 0 then
                                            SQL =SQL & " and toppinggroupid=" & mipallowtoppings     
                                        else
                                            SQL =SQL & " and toppinggroupid in (" & miallowtoppings & ","& mipallowtoppings  &" ) "
                                        end if
...
<div class="panel panel-primary"  id="basket"  >
  ...
  <div class="panel-body" style="padding:15px 8px 15px 8px;" id="footerbasket">
                        ...
                        <form id="CheckOutForm" action="CheckOut_new.asp?id_r=<%=vRestaurantId%>" method="post">                
                ...
                <fieldset class="gllpLatlonPicker" id="gllpLatlonPicker1">
		         ...
		         <div id="showdistance" align="center" style="clear:both;"></div>					
			                    <div class="delivery_info alert alert-danger" style="display:visible;" id="delivery-info">    
                                      ...
                                      <div class="alert alert-danger" id="missingPostcodeAlert" style="display:none;margin: 2px 8px 2px 2px;"><span style="color:#49cb29;font-weight:bold;">Check</span> delivery is available, then click <span style="color:#49cb29;font-weight:bold;">Place Order</span> to continue</strong>
								        <br>
        </div>


                   <div class="alert alert-danger" style="margin: 2px 8px 2px 2px;" id="missingPostcodeAlert2">We don't deliver to that postcode.
        </div>
           <div class="alert alert-danger" id="missingPostcodeAlert3" style="margin: 2px 8px 2px 2px;">Postcode must contain a space.
        </div>
                                </div>
        ...
        <div class="panel-body">           
             ...
             jscnt=jscnt+1
                        ...
                        End If
                          ...
                          %>
                            <tr <% if objRds("DayOfWeek") = Weekday(Date(),vbMonday)  then %> style="font-weight:bold;" <% end if %>>
                                <td style="width: 30px">
...
<script type="text/javascript">
    ...
    $(".no-border").filter("[parent='0']").each(function(){
        ...
        var newline = '<div class="product-line  no-border">';
    ...
    if($("[name=PreCollectionOpen]").val()=="false"){
        ...
        /* if (jsDate[key].max<jsDate[key].min) {
                  ...
                  isopen=1;
                                    ...
                                    if (nocollection == 0 && jsDate[key].min <= _time &&  maxTime1  >= (_time + (offsetmins * 60000)) ) {
            ...
            for (key2 in jsDate) {
                    if (jsDate[key2].d==dayprev && jsDate[key2].collection=="y") {
                                    if ( jsDate[key2].max<jsDate[key2].min ) {
                                            if ( (_time + (offsetmins *60000)) <= jsDate[key2].max) {
        ...
        function CheckDeliveryTime() {
            ...
            for (key in jsDate) {
                ...
                if (jsDate[key].d==days ) {
                    ...
                    if(maxTime1 < jsDate[key].min)
                        maxTime1 = jsDate[key].max + 24 * 60 * 60000;
                    if (nodelivery ==0 && jsDate[key].min <= _time &&  maxTime1 >= ( _time +  (offsetmins * 60000)) ) {
                        if((jsDate[key].min + offsetmins * 60000) > _time){
                            OpenTime = new Date();
                            OpenTime.setTime(jsDate[key].min + offsetmins * 60000); 
                            isEarly = 1;
...
<div id="loginModal" class="modal fade">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <h4>Login</h4>
                <a data-dismiss="modal" class="modal-close"><span class="glyphicon glyphicon-remove"></span></a>
            </div>
            <div class="modal-body">
...
<!-- End reveiws Modal -->
    ...
    <input type="hidden" value="<%=sisopen %>"  name ="sisopen" />
<script type="text/javascript">
    $(window).load(function() {
        $(".loader").fadeOut("slow");
        $("#wholepage").show();
             ...
             for (key in jsDate) {
                    if (jsDate[key].d==dt1Day) {
                        ...
                        if(openMaxTime <  openMinTime)
                         ...
                         if(mode=="d"){
                            if( currenttime < jsDate[key].min && minTime == 0 && jsDate[key].delivery=="y" ){
                        ...
                        {
                              if( currenttime < jsDate[key].min && minTime == 0 && jsDate[key].collection=="y" ){
                                       minTime =  jsDate[key].min;
                                       maxTime =  jsDate[key].max;
                             }    
                             if(currenttime > openMinTime && currenttime < openMaxTime)
...
//        }
    ...
    // render Expected time for Delivery 
        ...
        if(CurrentDate != nextOpeningTime.getDate() || ((dt1.getHours() < nextOpeningTime.getHours() || (dt1.getHours() == nextOpeningTime.getHours() && dt1.getMinutes() < nextOpeningTime.getMinutes()))) ){
            isExpected = true;
