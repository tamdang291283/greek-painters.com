...
if  objRds.BOF then
  ...
  Response.End
        ...
        ' Task 277
        if lcase(objRds("Payment_Status")& "") = "paid" then
              if trim(objRds("PaymentSurcharge") & "") = "0" then
                         If CREDITCARDSURCHARGE & "" <> "" And CREDITCARDSURCHARGE & "" <> "0" Then
                            dim PaymentSurcharge1: PaymentSurcharge1 = cdbl(CREDITCARDSURCHARGE)
                            dim OrderTotal1 : OrderTotal1 =   cdbl(objRds("OrderTotal")) + PaymentSurcharge1
                            objCon.execute("Update orders set PaymentSurcharge=" & PaymentSurcharge1 & ", OrderTotal=" & OrderTotal1 & " where ID=" & OrderResID)
                            SendEmailV2 "issue found in order " & OrderResID & " for storeID " & session("restaurantid"), "Order " & OrderResID & " haven't charged payment surchange!" , CONFIRMATION_EMAIL_ADDRESS
                         end if
              end if
        end if
        ' Task 277
