...
<%= objRds("FoodType") %>
	...
	if objRds("vouchercodediscount") <> 0 or objRds("vouchercode") & "" <> ""  then
        ...
        ' Task 277
        WriteLog server.MapPath("PaymentSurcharge.txt")," before checkout.asp  OrderID = "  &  vOrderId & " PaymentSurcharge "  & objRds("PaymentSurcharge")
        objRds("PaymentSurcharge") = 0
        ' Task 277
        WriteLog server.MapPath("PaymentSurcharge.txt")," after checkout.asp  OrderID = "  &  vOrderId & " PaymentSurcharge "  & objRds("PaymentSurcharge")
       ...
       Dim deliveryLat,deliveryLng
        ...
        set objRds =  nothing
        'objCon.Close 
        vOrderTotal = vOrderSubTotal + vOrderShipTotal + serviceChargeAmount + TaxAmount + TipAmount
    %>
		...
		<%end if%>
						...
						<% 
                            ...
                            IF NOCHEX="Yes" THEN
                              ...
                              <div class="block-direct" <%=popoverAttr %>>
							        <button  <%=disableButton %>   type="submit" name="payment_type" value="nochex"  class="btn btn-primary btn-block" >Pay by Debit/Credit Card (Nochex)<br>(<%=CURRENCYSYMBOL%><%=CREDITCARDSURCHARGE%> service charge)</button> 
                            </div>
					    <%end if%>		
					<% IF PAYPAL="Yes" THEN
                        divCount = divCount + 1 
                        ...
                        %>
                              ...
                              <button <%=disableButton %>  type="submit" name="payment_type" value="paypal"  class="btn btn-primary btn-block btn-paypal">
								 <br><br>(<%=CURRENCYSYMBOL%><%=CREDITCARDSURCHARGE%> service charge)</button>
                            </div>
                    ...
                    <% IF WORLDPAY="Yes" THEN
                        ...
                        %>
                              ...
                              <div class="block-direct" <%=popoverAttr %>>
                              <button <%=disableButton %>  type="submit" name="payment_type" value="worldpay"  class="btn btn-primary btn-block btn-worldpay" ><!--Pay by Debit/Credit Card (Worldpay)--><br><br>(<%=CURRENCYSYMBOL%><%=CREDITCARDSURCHARGE%> service charge)</button>
                            </div>
					<%end if%>	
                                    ...
                                    if  ISSTRIPE = "Yes" then
                                        ...
                                        <%
                                             ...
                                             %>                                               
                                                <div class="block-direct" <%=popoverAttr %>>
                                                  <button <%=disableButton %>  type="submit" name="payment_type" value="stripe"  class="btn btn-primary btn-block btn-stripe" ><br><br>(<%=CURRENCYSYMBOL%><%=CREDITCARDSURCHARGE%> service charge)</button>
                                                </div>
                                    ...
                                    <!-- #include file="Payments/stripe/stripepayment.asp" -->
                                        <div id="idsurchage" style="display:none;color:grey;">(<%=CURRENCYSYMBOL%><%=CREDITCARDSURCHARGE%> service charge)</div>
                                          <%end if %>
                                    </div>
                                     <% 
