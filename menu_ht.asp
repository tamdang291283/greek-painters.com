...
<% end if %>
  ...
  <script type="text/javascript">
   ...
   var individualpostcodeschecking ;
	...
	<link href="<%=SITE_URL %>css/style.css?v=1.4" rel="stylesheet">	
								            ...
								            If Photo <> "" Then 
								               ...
								               photo=1%>
                                                   <div  class="product10w photo" data-toggle="modal" data-target="#lightbox">  
                                                        <img data-src="<%=SITE_URL %>Images/<%=vRestaurantId %>/<%= objRds_MenuItem("Photo")%>" class="img-rounded lazy" alt="<%= MenuItemName%>" style="vertical-align: top;max-width:40px;" /> 
                                                            <div class="overlay">
								...
								if (miallowtoppings & "" <> "0" and trim( miallowtoppings & "") <> "") or ( mipallowtoppings & "" <> "0" and trim( mipallowtoppings & "") <> "")  then
                                        ...
                                        while not objRds_toppings_Group.EOF 
                                                ...
                                                While NOT objRds_toppings.Eof 
                                                    dishtoppingstext = dishtoppingstext &  "<input type=""checkbox"" class=""topping""   toppinggroup=""topping_" & objRds_toppings_Group("ID") &""" name=""" & objRds_toppings("topping") & """ value=""" & objRds_toppings("id") & """ data-group=""toppings" & vMenuItemId & "-" & PropertyId & """> " & objRds_toppings("topping") & " - " & CURRENCYSYMBOL & FormatNumber(objRds_toppings("toppingprice"),2) & "<BR>"
...
<script type="text/javascript">   
    ...
    function CheckDistance() {
        ...
        CheckPointInDeliveryZone($("#validate_pc").val());
        if (isInDeliveryZone == 0) {
            $("#missingPostcodeAlert").hide();
            $("#missingPostcodeAlert2").show();
            $("#missingPostcodeAlert3").hide();
            return false;
        }
        else if (isInDeliveryZone == -1) return false;
        <%if individualpostcodeschecking=0 then%>
         CheckDistanceLatLng();
        return false;
...
<%end if%>
 ...
 function StoreToC(obj,cname)
 {
    ...
    return "";
     ...
     var isInDeliveryZone = -1;
   function CheckPointInDeliveryZone(address){
         isInDeliveryZone = -1;
         <% if s_DeliveryZonesPath & "" = "" Then %>  isInDeliveryZone = 1; return true; <% end if %>
                ...
                geocoder.geocode({"address":address }, function(results, status) {
                    if (status == google.maps.GeocoderStatus.OK && results[0]) {
                       ...
                       deliveryLng = tempLng;
                        ...
                        if(deliveryLat == '' || deliveryLng == '' ) return true;
                     var deliveryZones = JSON.parse('<%=s_DeliveryZonesPath %>');
                      for(var i = 0; i < deliveryZones.Zones.length; i++){
                        ...
                        console.log('latlng:' + deliveryLat +'|' +deliveryLng);
                      if(google.maps.geometry.poly.containsLocation(new google.maps.LatLng(deliveryLat, deliveryLng), deliveryZone)) isInDeliveryZone = 1;
           
                    }
                     console.log('return false1;' + isInDeliverZone);
                    isInDeliveryZone = 0;
                    CheckDistance();
            }
        else {console.log('return false2;');isInDeliveryZone = 0;}
      });
