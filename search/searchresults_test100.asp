...
<status>OK</status>
<origin_address>P.O. Box 793, Brooklyn, NY 11207, USA</origin_address>
<destination_address>339 Hicks St, Brooklyn, NY 11201, USA</destination_address>
<row>
<element>
<status>OK</status>
<duration>
...
<%
    ...
    function GetDistance(byval lon1, byval lat1,byval lon2, byval lat2)
        dim result : result = "-1"
          Dim objXMLDoc, url, docXML, lat, lng, mapref
    
          url = "https://maps.googleapis.com/maps/api/distancematrix/xml?units=metric&origins=lat1,lon1&destinations=lat2,lon2&key=AIzaSyAvyfg017v5c_Wi2hQykmsv8VpS6tNaQoM"
          url = Replace(url, "lon1", Server.URLEncode(lon1))
          url = Replace(url, "lat1", Server.URLEncode(lat1))
          url = Replace(url, "lon2", Server.URLEncode(lon2))
          url = Replace(url, "lat2", Server.URLEncode(lat2))
            ...
            Set objXMLDoc = Server.CreateObject("MSXML2.ServerXMLHTTP.3.0")
              ...
              If objXMLDoc.status = 200 Then
                ...
                If UCase(docXML.documentElement.selectSingleNode("/DistanceMatrixResponse/status").Text) = "OK" Then                   
                  result = docXML.documentElement.selectSingleNode("/DistanceMatrixResponse/row/element/distance/text").Text  
                  if instr(result," m") > 0 then
                      result =  replace(result," m","")
                      result = cdbl(result) / 1000   
                      
                  end if           
                    result  = replace(result&""," km","")  
                End If              
              End If
...
Function GetXML(addr)
  ...
  'config include file or passed in as a function parameter.
    'The+Carriage+Drive%2C+Hadfield%2C+Glossop+SK13+1PP%2C+UK
...
<div class="container">
	<div class="row clearfix">
	<div class="col-md-3"></div>	<div class="col-md-6"><form action="searchresults_test100.asp" method="get"><div class="input-group custom-search-form">
...
Set objRds_cmd = Server.CreateObject ("ADODB.Command")
    objRds_cmd.ActiveConnection = sConnStringcms
           ...
           ' objCon.Open sConnStringcms
			if request.querystring("searchtype") = "postcode"  then
                objRds_cmd.CommandText =  "SELECT *  FROM BusinessDetails with(nolock)  " 
			end if
			if request.querystring("searchtype") = "dish"  then
			    if dishids<>"" then
    ...
    if request.querystring("searchtype") = "postcode" then
        If objRds("longitude") & "" <> "" AND objRds("latitude") & "" <> "" Then
           ' Response.Write("lat1 " & coords(0) & " lon1 " & coords(1) & " [] lat2 " & objRds("latitude") & "  lon2 " & objRds("longitude") & "<br/>")
            '' Check long lat exists in database 
             set objRds2 = Server.CreateObject("ADODB.Recordset")
             objRds2.Open "SELECT top 1 Distance  from Customer_Distance with(nolock) where IdBusinessDetail=" &  objRds("ID") & " and long_Cus='" & coords(1) & "' and lat_Cus='" &  coords(0) & "'", objCon
             if not objRds2.EOF then
                  ddd =  objRds2("Distance") 
             else
                  ddd =  GetDistance(coords(1), coords(0),objRds("longitude"), objRds("latitude") )      
                  Dim SQL:SQL=""
                  SQL  = "INSERT INTO Customer_Distance(long_store,lat_store,long_Cus,lat_Cus,postcode,IdBusinessDetail,Distance) "
                  SQL = SQL & "Values('" & objRds("longitude") &"','" & objRds("latitude") &"','" &coords(1)&"','" &coords(0)&"','"& request.querystring("address") & "',"& objRds("ID") &","&ddd&") "    
     
                  objCon.execute(SQL)
             end if
            objRds2.close()
            set objRds2 = nothing
             ' round(distance(coords(0), coords(1), objRds("latitude"), objRds("longitude"), "M"),1)
            'Response.Write("ddd " & ddd & "<br/>")
        End If
    else 
        if request.querystring("lat")<>"" AND request.querystring("lat")<>"0" then
            If objRds("longitude") & "" <> "" AND objRds("latitude") & "" <> "" Then
                ddd=GetDistance(request.querystring("long"), request.querystring("lat"), objRds("longitude"), objRds("latitude"))  
                'round(distance(request.querystring("lat"), request.querystring("long"), objRds("latitude"), objRds("longitude"), "M"),1)
            End If
        else
            ddd=0
        end if
...
if ( ddd<=objRds("DeliveryMaxDistance") and resultcoderesponse ="" ) or 1 = 1 then
...
<%
 if resultcoderesponse =  "" then   
         if request.querystring("searchtype") = "postcode" then  
               ' Response.Write(GetDistance(coords(1), coords(0), objRds("longitude"), objRds("latitude")) & " Miles<br>")
