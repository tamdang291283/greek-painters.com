...
'objCon.Open sConnStringcms
Set objRds = Server.CreateObject("ADODB.Recordset") 
objRds.Open "SELECT DISTINCT Orders.Email, Count(Orders.OrderTotal) AS CountOfOrderTotal, DateDiff(minute,[orderdate],getdate()), Orders.IdBusinessDetail, Orders.acknowledged FROM view_paid_orders Orders GROUP BY Orders.Email, DateDiff(minute,[orderdate],getdate()), Orders.IdBusinessDetail, Orders.acknowledged HAVING (((DateDiff(minute,[orderdate],getdate()))<=1) AND ((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND ((Orders.acknowledged)=1)) ORDER BY Count(Orders.OrderTotal) DESC;", objCon
...
'objCon.Open sConnStringcms
Set objRds = Server.CreateObject("ADODB.Recordset") 
objRds.Open "SELECT TOP 20 OrderItems.MenuItemId, MenuItems.Name, Orders.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems    INNER JOIN (OrderItems    INNER JOIN view_paid_orders Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId GROUP BY OrderItems.MenuItemId, MenuItems.Name, Orders.IdBusinessDetail, (((DateDiff(minute,[orderdate],getdate())))), Orders.acknowledged HAVING (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff(minute,[orderdate],getdate())))))<=3) AND ((Orders.acknowledged)=1)) ORDER BY OrderItems.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
'objCon.Open sConnStringcms
Set objRds = Server.CreateObject("ADODB.Recordset") 
objRds.Open "SELECT TOP 20 OrderItems.MenuItemId, MenuItems.Name, Orders.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems    INNER JOIN (OrderItems    INNER JOIN view_paid_orders Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId GROUP BY OrderItems.MenuItemId, MenuItems.Name, Orders.IdBusinessDetail, (((DateDiff(minute,[orderdate],getdate())))), Orders.acknowledged HAVING (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff(minute,[orderdate],getdate())))))<=1) AND ((Orders.acknowledged)=1)) ORDER BY OrderItems.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
'objCon.Open sConnStringcms
set objRds = Server.CreateObject("ADODB.Recordset")
objRds.Open "SELECT TOP 20 OrderItems.MenuItemId, MenuItems.Name, Orders.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems    INNER JOIN (OrderItems    INNER JOIN view_paid_orders Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId GROUP BY OrderItems.MenuItemId, MenuItems.Name, Orders.IdBusinessDetail, (((DateDiff(day,[orderdate],getdate())))), Orders.acknowledged HAVING (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff(day,[orderdate],getdate())))))<=7) AND ((Orders.acknowledged)=1)) ORDER BY OrderItems.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
