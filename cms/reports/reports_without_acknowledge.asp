...
'objCon.Open sConnStringcms
    ...
    objRds.Open "SELECT DISTINCT Orders.Email, Count(Orders.OrderTotal) AS CountOfOrderTotal, Orders.IdBusinessDetail FROM view_paid_orders Orders WHERE  (((DateDiff(minute,[orderdate],getdate()))<=1) AND ((Orders.IdBusinessDetail)=" & Session("MM_id") & ") ) GROUP BY Orders.Email, Orders.IdBusinessDetail  ORDER BY Count(Orders.OrderTotal) DESC;", objCon ', Orders.acknowledged  ,DateDiff(minute,[orderdate],getdate()),
...
'objCon.Open sConnStringcms
 Set objRds = Server.CreateObject("ADODB.Recordset")              
objRds.Open "SELECT TOP 20  MenuItems.Name, Count(MenuItems.Name) AS CountOfName FROM MenuItems    INNER JOIN (OrderItems    INNER JOIN view_paid_orders Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId  where (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff(minute,[orderdate],getdate())))))<=3) )  GROUP BY  MenuItems.Name ORDER BY MenuItems.Name;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
'objCon.Open sConnStringcms
  Set objRds = Server.CreateObject("ADODB.Recordset")              
objRds.Open "SELECT TOP 20  MenuItems.Name, Count(MenuItems.Name) AS CountOfName FROM MenuItems    INNER JOIN (OrderItems    INNER JOIN view_paid_orders Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId where (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff(minute,[orderdate],getdate())))))<=1) )  GROUP BY MenuItems.Name ORDER BY MenuItems.Name;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
'objCon.Open sConnStringcms
Set objRds = Server.CreateObject("ADODB.Recordset")  
objRds.Open "SELECT TOP 20  MenuItems.Name,  Count(MenuItems.Name) AS CountOfName FROM MenuItems    INNER JOIN (OrderItems    INNER JOIN view_paid_orders Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId where (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff(day,[orderdate],getdate())))))<=7) )  GROUP BY  MenuItems.Name  ORDER BY MenuItems.Name;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
