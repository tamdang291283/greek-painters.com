...
<br><br>
    ...
    <% if 1 = 2 then 'remove customer infor section %>
...
objCon.Close
objCon.Open sConnStringcms
'objRds.Open "SELECT DISTINCTROW Orders.Email, Count(Orders.OrderTotal) AS CountOfOrderTotal, Orders.IdBusinessDetail FROM Orders GROUP BY Orders.Email, DateDiff('m',[orderdate],Now()), Orders.IdBusinessDetail, Orders.acknowledged HAVING (((DateDiff('m',[orderdate],Now()))<=1) AND ((Orders.IdBusinessDetail)=" & Session("MM_id") & ") ) ORDER BY Count(Orders.OrderTotal) DESC;", objCon ', Orders.acknowledged  ,DateDiff('m',[orderdate],Now()),
    objRds.Open "SELECT DISTINCTROW Orderslocal.Email, Count(Orderslocal.OrderTotal) AS CountOfOrderTotal, Orderslocal.IdBusinessDetail FROM Orderslocal WHERE  (((DateDiff('m',[orderdate],Now()))<=1) AND ((Orderslocal.IdBusinessDetail)=" & Session("MM_id") & ") ) GROUP BY Orderslocal.Email, Orderslocal.IdBusinessDetail  ORDER BY Count(Orderslocal.OrderTotal) DESC;", objCon ', Orders.acknowledged  ,DateDiff('m',[orderdate],Now()),
  ...
  Set objCon2 = Server.CreateObject("ADODB.Connection")
   ...
   objCon2.Open sConnStringcms
                        objRds2.Open "SELECT * FROM ORDERSlocal where email='" & objRds("Email")& "'" , objCon
...
objCon.Close
objCon.Open sConnStringcms
objRds.Open "SELECT TOP 20 OrderItemslocal.MenuItemId, MenuItems.Name, Orderslocal.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems INNER JOIN (OrderItemslocal INNER JOIN Orderslocal ON OrderItemslocal.OrderId = Orderslocal.ID) ON MenuItems.Id = OrderItemslocal.MenuItemId GROUP BY OrderItemslocal.MenuItemId, MenuItems.Name, Orderslocal.IdBusinessDetail, (((DateDiff('m',[orderdate],Now())))), Orderslocal.acknowledged HAVING (((Orderslocal.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff('m',[orderdate],Now())))))<=3) ) ORDER BY OrderItemslocal.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
objCon.Close
objCon.Open sConnStringcms
objRds.Open "SELECT TOP 20 OrderItemslocal.MenuItemId, MenuItems.Name, Orderslocal.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems INNER JOIN (OrderItemslocal INNER JOIN Orderslocal ON OrderItemslocal.OrderId = Orderslocal.ID) ON MenuItems.Id = OrderItemslocal.MenuItemId GROUP BY OrderItemslocal.MenuItemId, MenuItems.Name, Orderslocal.IdBusinessDetail, (((DateDiff('m',[orderdate],Now())))), Orderslocal.acknowledged HAVING (((Orderslocal.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff('m',[orderdate],Now())))))<=1) ) ORDER BY OrderItemslocal.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
objCon.Close
objCon.Open sConnStringcms
objRds.Open "SELECT TOP 20 OrderItemslocal.MenuItemId, MenuItems.Name, Orderslocal.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems INNER JOIN (OrderItemslocal INNER JOIN Orderslocal ON OrderItemslocal.OrderId = Orderslocal.ID) ON MenuItems.Id = OrderItemslocal.MenuItemId GROUP BY OrderItemslocal.MenuItemId, MenuItems.Name, Orderslocal.IdBusinessDetail, (((DateDiff('d',[orderdate],Now())))), Orderslocal.acknowledged HAVING (((Orderslocal.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff('d',[orderdate],Now())))))<=7) ) ORDER BY OrderItemslocal.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
