...
$('#results').load('ajax-totalorderlocal.asp');
			...
			<div class="row clearfix">
				<div class="col-md-4 column">
				  ...
				  objCon.Open sConnStringcms
                       ...
                       objRds.Open "SELECT * FROM ORDERSlocal where  (paymenttype='Paypal-Paid' or paymenttype='NoChex-Paid' or paymenttype='Worldpay-Paid' or paymenttype='Cash on Delivery') and  IdBusinessDetail=" & Session("MM_id") & " and (IIf ( IsNull ( [CreationDate] ) , '',Format([CreationDate],'dd/mm/yyyy')) ='" & date() & "') and IIf ( IsNull ( [subtotal] ) , '0',subtotal)  > 0 and IIf ( IsNull ( [firstname] ) , '',firstname)  <> '' ORDER BY id desc" , objCon
total=0
totalinccancelled=0
ordercnt=0
ordercancelled=0
				  ...
				  objCon.Open sConnStringcms
                       ...
                       <tr>
						...
						<td><span class="pull-right"><a href="https://www.google.co.uk/maps?q=<%= objRds("Address") %>,<%= objRds("PostalCode") %> " target="_blank"><%= objRds("PostalCode") %></a></span>
							...
							<small> Ordered on: <%= objRds("CreationDate") %>&nbsp;&nbsp;&nbsp;Order Value: <%=CURRENCYSYMBOL%><%=FormatNumber(objRds("OrderTotal"),2) %></small>
						</td>
						<td>
						<%if objRds("cancelled")=-1 then%>
						<b>CANCELLED:</b><br>
...
<div class="modal fade" id="printModal" tabindex="-1" role="dialog" aria-labelledby="printModalLabel" aria-hidden="true">
  ...
  <div class="form-group">
      ...
      <% if INRESTAURANTEPSONPRINTERIDLIST & "" <> "" Then %>
    <label for="exampleInputEmail1">Select Printer:</label>
      <% Dim arrEpsonPrinterIDList
          arrEpsonPrinterIDList = Split(INRESTAURANTEPSONPRINTERIDLIST,";")
...
function ReloadOrderList(){
    $.ajax({
  url: "ajax-totalorderdetaillocal.asp"
