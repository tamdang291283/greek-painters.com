...
<div class="row clearfix">
		<div class="col-md-12 column">
			<%
                        ...
                        objRds.Open "SELECT * FROM menuitems where id=" & request.querystring("id"), objCon

%>
			<ol class="breadcrumb">
<li><a href="menu.asp">Main Menu</a></li>
  <li><a href="menu-categories-sub.asp?id=<%=objRds("id")%>"><%= objRds("name") %></a></li>

</ol>
		<a href="menu-sub-add.asp?catid=<%=objRds("id")%>"  class="btn btn-primary pull-right">ADD</a><h1>Menu Items (sub)</h1>
		<p>Click add to assign new products to this sub category.</p>
			<table class="table table-hover table-condensed table-striped">
				...
				<tbody name="bodysub" id="<%=request.querystring("id") %>">
				
				<%  objRds.Close
                   set objRds = nothing
				    Set objRds = Server.CreateObject("ADODB.Recordset") 
                        objRds.Open "SELECT * FROM menuitemproperties where IdMenuitem=" & request.querystring("id") & " order by i_displaysort,id " , objCon
			...
			<a class="btn btn-primary btn"  href="menu-sub-edit.asp?id=<%=objRds("id")%>&catid=<%=objRds("id")%>">
EDIT
</a>
