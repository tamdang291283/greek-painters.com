...
Set objCon = Server.CreateObject("ADODB.Connection")
    ...
    function  WriteSelected(byval sel1, byval sel2)
        dim result : result = ""
        if  sel1 = sel2  then
...
<form class="form-inline mt20" id="searchform" action="history_v2.asp">
...
<div class="col-md-2">
    <!--<input type="text" class="form-control datepicker" id="endDate" name="endDate" placeholder="End date" value="<%=ed%>" size="10">-->
    <select name="slpayment" class="form-control" >
        <option value="" <%=WriteSelected("",Request.QueryString("slpayment") & "") %>>All</option>
        <% if PAYPAL="Yes" then %>
        <option value="paypal" <%=WriteSelected("paypal",Request.QueryString("slpayment") & "") %>>Paypal</option>
        <% end if %>
         <% if NOCHEX="Yes" then %>
        <option value="nochex" <%=WriteSelected("nochex",Request.QueryString("slpayment") & "") %>>Nochex</option>
        <% end if %>
         <% if ISSTRIPE="Yes" then %>
        <option value="stripe" <%=WriteSelected("stripe",Request.QueryString("slpayment") & "") %>>Stripe</option>
          <% end if %>
         <% if WORLDPAY="Yes" then %>
        <option value="worldpay" <%=WriteSelected("worldpay",Request.QueryString("slpayment") & "") %>>Worldpay</option>
            <% end if %>
        <% if enable_CashPayment = "Yes" then  %>
        <option value="Cash on delivery" <%=WriteSelected("Cash on delivery",Request.QueryString("slpayment") & "") %>>Cash</option>
        <%end if %>
    </select>
</div>
...
sql="SELECT * FROM view_paid_orders where {STATUS_CONDITION} and IdBusinessDetail=" & Session("MM_id") 
...
sql=sql & " AND  [OrderDate]  >='" & startdate_sql & " 00:00:00" & "'"
...
if Request.QueryString("slpayment") & "" <> "" then
   sql = sql & " and PaymentType ='" & lcase(Request.QueryString("slpayment") & "")   & "'"
end if

sql = sql & " ORDER BY OrderDate desc"


objRds.Open Replace(sql,"{STATUS_CONDITION}","  cancelled <> 1 ")   , objCon,1
...
<%=objRds("id")%>
						...
						%>
						<div align="center"><%=replace(replace(objRds("paymenttype"),"-Paid",""),"-paid","") %></div>
						<%end if%></td>
					<td><%if objRds("paymenttype")="Cash on Delivery" then
					%>
						<div align="center"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span></div>
...
if abs(page)>10 then%>
<li><a href="history_v2.asp?freetext=<%=request.querystring("freetext")%>&startDate=<%=sd%>&endDate=<%=ed%>&page=<%=1%>" class="">1</a>.....</li>
<%end if%>
...
<li class="<%if abs(page)=abs(g) then%>active<%else%><%end if%>"><a href="history_v2.asp?freetext=<%=request.querystring("freetext")%>&startDate=<%=sd%>&endDate=<%=ed%>&page=<%=g%>" ><%=g%></a></li><%end if%><%next%><%
if abs(page)<round(abs((totalrecords/pagesize))+0.5)-10 then%>
<li>...<a href="history_v2.asp?freetext=<%=request.querystring("freetext")%>&startDate=<%=sd%>&endDate=<%=ed%>&page=<%=round(abs((totalrecords/pagesize))+0.5)%>" class=""><%=round(abs((totalrecords/pagesize))+0.5)%></a></li>
<%end if%>
  </ul>
</nav>
