...
intervalID = setInterval(function () {
    $('#results').load('ajax-neworder2-with.asp');
    if ($("#results" ).text() != "" && $( "#completetotal" ).text() != "" && parseInt($( "#results" ).text()) > parseInt($( "#completetotal" ).text())) {
$( "#refreshspace" ).hide();
$( "#refresh" ).show();
...
<%if objRds("cancelled")=1 or objRds("acknowledged")=1  then%><%else%><button class="btn btn-primary btn-danger cancel" data-toggle="modal" data-target="#myModal" data-id=" <%= objRds("id") %>" data-email=" <%= objRds("email") %>" >Cancel</button>	<a href="../exe.asp?action=acknowledge&id=<%=objRds("id")%>&email=<%=objRds("email")%>&page=outstanding-with-acknowledgement" class="btn btn-success"">Acknowledge</a><%end if%>
...
<a href="../exe.asp?action=collected&id=<%=objRds("id")%>&email=<%=objRds("email")%>&page=outstanding-with-acknowledgement" class="btn btn-success"">Collected</a>
<%else%>
<a href="../exe.asp?action=outfordelivery&id=<%=objRds("id")%>&email=<%=objRds("email")%>&page=outstanding-with-acknowledgement" class="btn btn-success"">Out for delivery</a>
<%end if%>
<%end if%>
 <%else%>
  ...
  <div class="form-group">
   ...
   <textarea class="form-control" name="cancelledreason" id="cancelledreason" rows="3"></textarea>
            ...
            <div class="modal-footer">
				...
				<input type="hidden" name="page" id="page" value="outstanding-with-acknowledgement">
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
...
function ReloadOrderList(){
    $.ajax({
  url: "ajax-neworderdetail2-with.asp"
