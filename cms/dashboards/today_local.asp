		...
		<div class="col-md-12 column">
			<table class="table table-hover table-condensed table-striped">
				...
				<% 
				 ...
				 ' objCon.Open sConnStringcms
                    Set objRds = Server.CreateObject("ADODB.Recordset") 
                        objRds.Open "SELECT * FROM view_paid_orderslocal where    IdBusinessDetail=" & Session("MM_id") & " and   DateDiff(day,[CreationDate],'" & JXIsoDate(DateAdd("h",houroffset,now)) & "')=0 and   isnull(subtotal,0)  > 0 and isnull(firstname,0)   <> '' ORDER BY id desc" , objCon
                       ...
                       <tr>
						...
						<td><span class="pull-right"><a href="https://www.google.co.uk/maps?q=<%= objRds("Address") %>,<%= objRds("PostalCode") %> " target="_blank"><%= objRds("PostalCode") %></a></span>
							...
							<small> Ordered on: <%= formatDateTimeC(objRds("CreationDate")) %>&nbsp;&nbsp;&nbsp;Order Value: <%=CURRENCYSYMBOL%><%=FormatNumber(objRds("OrderTotal"),2) %></small>
						</td>
						<td>
