...
%>
	<script>
        ...
        {
            location.href = "exportorder_local.asp?start=" + $("#startDate").val() + "&end=" + $("#endDate").val() + "&freetext=" + $("#freetext").val();
        }
	</script>
<form class="form-inline mt20" id="searchform" action="history_v2_local.asp">
...
sql="SELECT * FROM view_paid_orderslocal where {STATUS_CONDITION} and IdBusinessDetail=" & Session("MM_id") 
...
startdate_sql=  startdate_month & "/" & startdate_day & "/" & startdate_year & " 00:00:01"
'startdate_sql=  startdate_day  & "/" & startdate_month  & "/" & startdate_year & " 00:00:01"
...
enddate_month=sss(1)
enddate_year=sss(2)
enddate_sql=  enddate_month & "/" & enddate_day & "/" & enddate_year & " 23:59:59"
'enddate_sql=  enddate_day  & "/" & enddate_month  & "/" & enddate_year & " 23:59:59"
...
orders=0
				...
				<thead>
					...
					<th>
						...
						<% if 1=2 Then %><th><div align="center">Cancelled</div></th> <% end if %>
                        <th><div align="center">
            ...
            <%
					   ...
					   <td> <%=formatDateTimeC(objRds("OrderDate"))%></td>
						<td>
						<a href="javascript:;"  data-toggle="modal" data-target="#myModalorder" data-remote="orderlocal.asp?id_o=<%=objRds("id")%>&id_r=<%=Session("MM_id")%>">
<%=objRds("id")%>
</a>	
						</td>
...
if abs(page)>10 then%>
<li><a href="history_v2_local.asp?freetext=<%=request.querystring("freetext")%>&startDate=<%=sd%>&endDate=<%=ed%>&page=<%=1%>" class="">1</a>.....</li>
<%end if%>
...
<li class="<%if abs(page)=abs(g) then%>active<%else%><%end if%>"><a href="history_v2_local.asp?freetext=<%=request.querystring("freetext")%>&startDate=<%=sd%>&endDate=<%=ed%>&page=<%=g%>" ><%=g%></a></li><%end if%><%next%><%
if abs(page)<round(abs((totalrecords/pagesize))+0.5)-10 then%>
<li>...<a href="history_v2_local.asp?freetext=<%=request.querystring("freetext")%>&startDate=<%=sd%>&endDate=<%=ed%>&page=<%=round(abs((totalrecords/pagesize))+0.5)%>" class=""><%=round(abs((totalrecords/pagesize))+0.5)%></a></li>
<%end if%>
  </ul>
</nav>
...
<script type="text/javascript">
    ...
    function DeleteMultiOrder(obj) {
        ...
        x0p(action, textwarning, buttontype , function (button, text) {
            if (button != "cancel") {
                $.ajax({
                    url: "deleteOrder.asp?from=local&action=" + mode + "&OrderID=" + listoforderid + "&r" + Math.random()
                })
                  .done(function (data) {
                      if (data == "OK") {
