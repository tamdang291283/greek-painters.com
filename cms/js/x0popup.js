	...
	**/
		...
		str += '<div id="x0popup" class="x0p ' + config.theme + (config.animation == false ? ' no-animation' : '') + (buttons.length == 0 ? ' no-button' : '') + '" style="' + generateStyle() + '">';
			str += '<div class="content">';
				str += textOnly ? '<div class="text-pure-wrapper">' : generateIcon() + '<div class="text-wrapper">';
					...
					str += '<div class="title" ' + sStyleTitle + '>' + config.title + '</div>';
					(config.text != null) && (str += '<div class="text" ' + sStyleTitle + '>' + (config.html ? config.text : htmlEncode(config.text)) + '</div>');
	...
	function generateIcon() {
		...
		switch(iconType) {
			...
			case 'warning':
				...
				break;
		    case 'delete':
		        str += '<i class="xi xi-delete"><span class="xi-delete-circle"></span><span class="xi-delete-line"></span></i>';
		        break;
			case 'custom':
	...
	function generateButtons() {
		...
		if(buttons == null) {
			...
			if (
				config.showCancelButton == true ||
				(config.showCancelButton != false && (config.type == 'delete' || config.type == 'input'))
			) {
			    buttons.push({
			        type: 'cancel',
			        key: 27
