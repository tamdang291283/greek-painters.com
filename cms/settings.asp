...
MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param41", 6, 1, 255, Request.Form("minimumamountforcardpayment"))   ' Currency
   ...
   If Request.Form("SMSOnAcknowledgement") = "" Then
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param42", 11, 1, 255, "False") 
Else
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param42", 11, 1, 255, Request.Form("SMSOnAcknowledgement")) 
End If  
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param43", 202, 1, 255, Request.Form("LocalPrinterURL"))
    
    If Request.Form("ShowRestaurantDetailOnReceipt") = "" Then
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param44", 11, 1, 255, "True") 
    Else
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param44", 11, 1, 255, Request.Form("ShowRestaurantDetailOnReceipt")) 
    End If  

     If Request.Form("PrinterFontSizeRatio") = "" Then
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param45", 5, 1, 255, 1)  'aDouble
    Else
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param45", 5, 1, 255, Request.Form("PrinterFontSizeRatio")) 
    End If  
     If Request.Form("ServiceChargePercentage") = "" Then
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param46", 5, 1, 255, 0)  'aDouble
    Else
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param46", 5, 1, 255, Request.Form("ServiceChargePercentage")) 
    End If   
     If Request.Form("InRestaurantServiceChargeOnly") = "" Then
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param47", 11, 1, 255, "false")  'bit
    Else
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param47", 11, 1, 255, Request.Form("InRestaurantServiceChargeOnly")) 
    End If  
     If Request.Form("IsDualReceiptPrinting") = "" Then
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param48", 11, 1, 255, "false")  'bit
    Else
        MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param48", 11, 1, 255, Request.Form("IsDualReceiptPrinting")) 
    End If    
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param49", 202, 1, 255, Request.Form("InRestaurantEpsonPrinterIDList"))
     MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param50", 202, 1, 255, Request.Form("BlockIPEmailList")) 
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param50", 5, 1, -1, MM_IIF(Request.Form("MM_recordId"), Request.Form("MM_recordId"), null)) ' adDouble
...
<p>Set a service charge of  [<input type="text" style="width:35px;" name="ServiceChargePercentage" <%if Recordset1.Fields.Item("ServiceChargePercentage").Value & "" <> "" then%>value="<%=Recordset1.Fields.Item("ServiceChargePercentage").Value %>"<% else %>value="0"<%end if%> />]% for all orders.</p>
<input type="checkbox" name="InRestaurantServiceChargeOnly" value="true" <%if  LCase(Recordset1.Fields.Item("InRestaurantServiceChargeOnly").Value & "") =  "true" then  %> checked <% end if %> /> Tick this box is Service Charges apply to In-Restaurant ordering ONLY.
...
Your URL for worldpay callback is <%=SITE_URL%>payments/worldpay/worldpay.asp<br>
...
<input type="radio" name="ShowRestaurantDetailOnReceipt" value="False" <%if LCase(Recordset1.Fields.Item("ShowRestaurantDetailOnReceipt").Value)="false" then%>checked<%end if%>> Remove &nbsp;&nbsp; <input type="radio" name="ShowRestaurantDetailOnReceipt" value="True" <%if Lcase(Recordset1.Fields.Item("ShowRestaurantDetailOnReceipt").Value)="true" then%>checked<%end if%>> Include
...
<p style="font-size:13px;"><b>Important:</b> ‘Printing Names’ will be printed in a seperate receipt ONLY IF ALL the order items have a ‘printing name’.  Otherwise a single receipt will print out as normal.
...
<input type="radio" name="IsDualReceiptPrinting" value="False" <%if LCase(Recordset1.Fields.Item("IsDualReceiptPrinting").Value)="false" then%>checked<%end if%>> Disable &nbsp;&nbsp; <input type="radio" name="IsDualReceiptPrinting" value="True" <%if Lcase(Recordset1.Fields.Item("IsDualReceiptPrinting").Value)="true" then%>checked<%end if%>> Enable
...
<input type="text" class="form-control" id="PrinterIDList" name="PrinterIDList" value="<%=(Recordset1.Fields.Item("PrinterIDList").Value)%>">
 ...
 <div class="form-group">
<label for="document name">In-restaurant EPSON printer ID list.</label>
<p>Printer ID is separated by ";". Printer ID MUST not contain "-". Ex: local_printer;local_printer;kitchen_printer;bar_printer</p>
...
<br />
      <div class="form-group" style="display:none;">
<label for="document name">EPSON Printer URL  for ‘In-Restaurant ordering’</label>
<p>URL to send print command to ,for EPSON printer. When user hit Make Order button under In-Restaurant ordering page. If this URL empty, then system will not print the order. Format: http://{PRINTER-IP-ADDRESS}/cgi-bin/epos/service.cgi?devid={PRINTER-ID}&timeout={TIMEOUT}. Ex: http://192.168.192.168/cgi-bin/epos/service.cgi?devid=local_printer&timeout=60000 </p>
...
Printer must be set to check <%=SITE_URL%>printers/Epson/printxml_v2.asp?id_r=<%= Recordset1.Fields.Item("ID").Value %><br>
...
Printer must be set to check <%=SITE_URL%>printers/Star/StarPrinting.asp?id_r=<%= Recordset1.Fields.Item("ID").Value %><br>
</div></div>
...
<br />
<input type="checkbox" name="SMSOnAcknowledgement" <% if Lcase(Recordset1.Fields.Item("SMSOnAcknowledgement").Value) = "true" then %> checked="checked" <% end if %> value="true"> &nbsp;Enable/Disable whether “Acknowledgement” button in back-end Dashboards send an SMS or not. (Tick to Enable)
<br />
<br />
