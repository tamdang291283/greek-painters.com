...
Dim PrintingURL
If EPSONJSPRINTERURL & "" <> "" Then
    PrintingURL = "../printers/epson/print_cms.asp"
Else
		...
		<div class="col-md-12 column">
			<div class="row clearfix">
				  ...
				  objCon.Open sConnStringcms
                       objRds.Open "SELECT * FROM ORDERS where (paymenttype='Paypal-Paid' or paymenttype='NoChex-Paid' or paymenttype='Worldpay-Paid' or paymenttype='Cash on Delivery') and IdBusinessDetail=" & Session("MM_id") & " and not(cancelled=-1 or outfordelivery=-1) and acknowledged=-1  ORDER BY DeliveryTime  asc" , objCon
total=0
ordercnt=0
ordercancelled=0
pending=0
...
<nav>
  ...
  <%if abs(page)>1 then%>
    <li>
      <a href="outstanding_v2.asp?s=<%=request.querystring("s")%>&sort=<%=request.querystring("sort")%>&page=<%=request.querystring("page")-1%>" aria-label="Previous">
...
<a href="outstanding_v2.asp?s=<%=request.querystring("s")%>&sort=<%=request.querystring("sort")%>&page=<%=g%>" ><%=g%></a></li>
<%next%>
    <%if abs(page)<round(abs((totalrecords/pagesize))+0.5) then%>
    <li>
      <a href="outstanding_v2.asp?s=<%=request.querystring("s")%>&sort=<%=request.querystring("sort")%>&page=<%=request.querystring("page")+1%>" aria-label="Next">
...
<a href="javascript::" onclick="openWin('<%=PrintingURL %>?id_o=<%=objRds("id")%>&id_r=<%=Session("MM_id")%>')" class="btn btn-default"">Print</a>
<%if objRds("deliverytype")="c" then%>
