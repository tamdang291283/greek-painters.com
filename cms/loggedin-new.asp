			...
			<div class="row clearfix">
				<div class="col-md-4 column">
				  ...
				  objCon.Open sConnStringcms
                       objRds.Open "SELECT * FROM ORDERS where (paymenttype='Paypal-Paid' or paymenttype='NoChex-Paid' or paymenttype='Worldpay-Paid' or paymenttype='Cash on Delivery') and IdBusinessDetail=" & Session("MM_id") & " and ([orderdate]=#" & now() & "# or (acknowledged=0 and cancelled=0 and outfordelivery=0))  ORDER BY id desc" , objCon
total=0
ordercnt=0
ordercancelled=0
pending=0
...
<a href="exe.asp?action=acknowledge&id=<%=objRds("id")%>&email=<%=objRds("email")%>" class="btn btn-success"">Acknowledge</a><%end if%>
...
<a href="exe.asp?action=collected&id=<%=objRds("id")%>&email=<%=objRds("email")%>" class="btn btn-success"">Collected</a>
<%else%>
<a href="exe.asp?action=outfordelivery&id=<%=objRds("id")%>&email=<%=objRds("email")%>" class="btn btn-success"">Out for delivery</a>
<%end if%>
<%end if%>
 <%else%>
