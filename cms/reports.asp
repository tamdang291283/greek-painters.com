...
objCon.Close
objCon.Open sConnStringcms
objRds.Open "SELECT DISTINCTROW Orders.Email, Count(Orders.OrderTotal) AS CountOfOrderTotal, DateDiff('m',[orderdate],Now()), Orders.IdBusinessDetail, Orders.acknowledged FROM Orders GROUP BY Orders.Email, DateDiff('m',[orderdate],Now()), Orders.IdBusinessDetail, Orders.acknowledged HAVING (((DateDiff('m',[orderdate],Now()))<=1) AND ((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND ((Orders.acknowledged)=-1)) ORDER BY Count(Orders.OrderTotal) DESC;", objCon
...
objCon.Close
objCon.Open sConnStringcms
objRds.Open "SELECT TOP 20 OrderItems.MenuItemId, MenuItems.Name, Orders.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems INNER JOIN (OrderItems INNER JOIN Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId GROUP BY OrderItems.MenuItemId, MenuItems.Name, Orders.IdBusinessDetail, (((DateDiff('m',[orderdate],Now())))), Orders.acknowledged HAVING (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff('m',[orderdate],Now())))))<=1) AND ((Orders.acknowledged)=-1)) ORDER BY OrderItems.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
...
objCon.Close
objCon.Open sConnStringcms
objRds.Open "SELECT TOP 20 OrderItems.MenuItemId, MenuItems.Name, Orders.acknowledged, Count(MenuItems.Name) AS CountOfName FROM MenuItems INNER JOIN (OrderItems INNER JOIN Orders ON OrderItems.OrderId = Orders.ID) ON MenuItems.Id = OrderItems.MenuItemId GROUP BY OrderItems.MenuItemId, MenuItems.Name, Orders.IdBusinessDetail, (((DateDiff('d',[orderdate],Now())))), Orders.acknowledged HAVING (((Orders.IdBusinessDetail)=" & Session("MM_id") & ") AND (((((DateDiff('d',[orderdate],Now())))))<=7) AND ((Orders.acknowledged)=-1)) ORDER BY OrderItems.MenuItemId;", objCon
atleast1=0
cntsame=0
cntitem=0
cntitemname=""
