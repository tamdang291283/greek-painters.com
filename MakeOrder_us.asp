...
objRds.Update 
 ...
 If LCase(SMSEnable) = "true" AND Lcase(SMSOnOrder) = "true" AND Request.Form("Phone") & "" <> "" AND SMSOnOrderContent & "" <> "" Then
    ...
    End If
   '  Response.Write(Request.Form("Phone") & "|" & SMSOnOrderContent & "|" & SendDate  & "|" & session("restaurantid"))
   'Response.End()
    Dim ActualPhoneNumber
    ActualPhoneNumber = ""

    If Request.Form("Phone") & "" <> "" Then
        ActualPhoneNumber = Request.Form("Phone")
...
<form action="<%= PAYPAL_URL %>" method="post">
    ...
    <input type="hidden" name="notify_url" value="<%= SITE_URL %>paypal-ipn-new.asp?r=<%=iRestaurantId %>"/>
...
response.write "email=" & iEmail & "<BR>"
    ...
    SendEmail MAIL_SUBJECT, SITE_URL & "Email.asp?notifyemail=Y&id_o=" & iItemNumber & "&id_r=" & iRestaurantId  , CONFIRMATION_EMAIL_ADDRESS
