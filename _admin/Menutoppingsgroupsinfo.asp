...
' ASPMaker configuration for Table Menutoppingsgroups
Dim Menutoppingsgroups

' Define table class
Class cMenutoppingsgroups
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "toppingsgroup", toppingsgroup)
	...
	Public Property Get HighlightName()
		HighlightName = "Menutoppingsgroups_Highlight"
	End Property
	...
	Public Property Get SqlSelect()
		SqlSelect = ew_IIf(zSqlSelect & "" <> "", zSqlSelect, "SELECT * FROM [dbo].[Menutoppingsgroups]")
	End Property
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "Menutoppingsgroupslist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "Menutoppingsgroupslist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("Menutoppingsgroupsview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("Menutoppingsgroupsview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
	...
	Public Function AddUrl(parm)
		If parm <> "" Then
			AddUrl = "Menutoppingsgroupsadd.asp?" & UrlParm(parm)
		Else
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("Menutoppingsgroupsedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("Menutoppingsgroupsadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("Menutoppingsgroupsdelete.asp", UrlParm(""))
	End Function
	...
	Public Sub LoadListRowValues(RsRow)
		ID.DbValue = RsRow("ID")
		toppingsgroup.DbValue = RsRow("toppingsgroup")
	...
	Sub RenderListRow()
		...
		' toppingsgroup
		toppingsgroup.ViewValue = toppingsgroup.CurrentValue
		toppingsgroup.ViewCustomAttributes = ""
		...
		' toppingsgroup
		toppingsgroup.LinkCustomAttributes = ""
		toppingsgroup.HrefValue = ""
		toppingsgroup.TooltipValue = ""
	...
	Public Sub RenderEditRow()
		...
		' toppingsgroup
		toppingsgroup.EditAttrs.UpdateAttribute "class", "form-control"
		toppingsgroup.EditCustomAttributes = ""
		toppingsgroup.EditValue = toppingsgroup.CurrentValue
		toppingsgroup.PlaceHolder = ew_HtmlEncode(ew_RemoveHtml(toppingsgroup.FldCaption))
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("toppingsgroup", toppingsgroup.ExportValue(Export))
		...
		If Not Doc.ExportCustom Then
			...
			If Doc.Horizontal Then ' Horizontal format, write header
				...
				If ExportPageType = "view" Then
					...
					If toppingsgroup.Exportable Then Call Doc.ExportCaption(toppingsgroup)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If Not Doc.ExportCustom Then
					...
					If ExportPageType = "view" Then
						...
						If toppingsgroup.Exportable Then Call Doc.ExportField(toppingsgroup)
...
'			End If
	...
	Public Property Get ID()
		If Not IsObject(m_ID) Then
			Set m_ID = NewFldObj("Menutoppingsgroups", "Menutoppingsgroups", "x_ID", "ID", "[ID]", "CAST([ID] AS NVARCHAR)", 3, 0, "[ID]", False, False, FALSE, "FORMATTED TEXT")
	...
	' Field toppingsgroup
	Private m_toppingsgroup

	Public Property Get toppingsgroup()
		If Not IsObject(m_toppingsgroup) Then
			Set m_toppingsgroup = NewFldObj("Menutoppingsgroups", "Menutoppingsgroups", "x_toppingsgroup", "toppingsgroup", "[toppingsgroup]", "[toppingsgroup]", 202, 0, "[toppingsgroup]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set toppingsgroup = m_toppingsgroup
	End Property
	...
	Public Property Get IdBusinessDetail()
		If Not IsObject(m_IdBusinessDetail) Then
			Set m_IdBusinessDetail = NewFldObj("Menutoppingsgroups", "Menutoppingsgroups", "x_IdBusinessDetail", "IdBusinessDetail", "[IdBusinessDetail]", "CAST([IdBusinessDetail] AS NVARCHAR)", 3, 0, "[IdBusinessDetail]", False, False, FALSE, "FORMATTED TEXT")
	...
	Public Property Get printingname()
		If Not IsObject(m_printingname) Then
			Set m_printingname = NewFldObj("Menutoppingsgroups", "Menutoppingsgroups", "x_printingname", "printingname", "[printingname]", "[printingname]", 202, 0, "[printingname]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get i_displaySort()
		If Not IsObject(m_i_displaySort) Then
			Set m_i_displaySort = NewFldObj("Menutoppingsgroups", "Menutoppingsgroups", "x_i_displaySort", "i_displaySort", "[i_displaySort]", "CAST([i_displaySort] AS NVARCHAR)", 3, 0, "[i_displaySort]", False, False, FALSE, "FORMATTED TEXT")
	...
	Private Sub Class_Terminate
		...
		If IsObject(m_toppingsgroup) Then Set m_toppingsgroup = Nothing
