...
' Define page object
Dim MenuItems_edit
Set MenuItems_edit = New cMenuItems_edit
Set Page = MenuItems_edit

' Page init processing
MenuItems_edit.Page_Init()

' Page main processing
MenuItems_edit.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Render event
MenuItems_edit.Page_Render()
%>
<!--#include file="header.asp"-->
<script type="text/javascript">
// Page object
var MenuItems_edit = new ew_Page("MenuItems_edit");
MenuItems_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = MenuItems_edit.PageID; // For backward compatibility
// Form object
var fMenuItemsedit = new ew_Form("fMenuItemsedit");
// Validate form
fMenuItemsedit.Validate = function() {
...
fMenuItemsedit.Form_CustomValidate = 
...
fMenuItemsedit.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fMenuItemsedit.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% MenuItems_edit.ShowPageHeader() %>
<% MenuItems_edit.ShowMessage %>
<form name="fMenuItemsedit" id="fMenuItemsedit" class="form-horizontal ewForm ewEditForm" action="<%= ew_CurrentPage %>" method="post">
<% If MenuItems_edit.CheckToken Then %>
<input type="hidden" name="<%= EW_TOKEN_NAME %>" value="<%= MenuItems_edit.Token %>">
<% End If %>
<input type="hidden" name="a_table" id="a_table" value="MenuItems">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<div>
<% If MenuItems.Id.Visible Then ' Id %>
	<div id="r_Id" class="form-group">
		<label id="elh_MenuItems_Id" class="col-sm-2 control-label ewLabel"><%= MenuItems.Id.FldCaption %></label>
		<div class="col-sm-10"><div<%= MenuItems.Id.CellAttributes %>>
<span id="el_MenuItems_Id">
<span<%= MenuItems.Id.ViewAttributes %>>
<p class="form-control-static"><%= MenuItems.Id.EditValue %></p>
</span>
</span>
<input type="hidden" data-field="x_Id" name="x_Id" id="x_Id" value="<%= Server.HTMLEncode(MenuItems.Id.CurrentValue&"") %>">
<%= MenuItems.Id.CustomMsg %></div></div>
	</div>
<% End If %>
...
Class cMenuItems_edit
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_edit")&"" <> "" Then
			MenuItems.CurrentAction = ObjForm.GetValue("a_edit") ' Get action code
			Call LoadFormValues() ' Get form values
		Else
			MenuItems.CurrentAction = "I" ' Default action is display
		End If

		' Check if valid key
		If MenuItems.Id.CurrentValue = "" Then Call Page_Terminate("MenuItemslist.asp") ' Invalid key, return to list

		' Validate form if post back
		If ObjForm.GetValue("a_edit")&"" <> "" Then
			If Not ValidateForm() Then
				MenuItems.CurrentAction = "" ' Form error, reset action
		...
		Select Case MenuItems.CurrentAction
			Case "I" ' Get a record to display
				...
				End If
			Case "U" ' Update
				MenuItems.SendEmail = True ' Send email on update success
		...
		' Render the record
		MenuItems.RowType = EW_ROWTYPE_EDIT ' Render as edit
	...
	Sub RenderRow()
		...
		ElseIf MenuItems.RowType = EW_ROWTYPE_EDIT Then ' Edit row
			...
			MenuItems.Id.EditValue = MenuItems.Id.CurrentValue
			MenuItems.Id.ViewCustomAttributes = ""
	...
	Function EditRow()
		...
		Else
			...
			' Field Code
			Call MenuItems.Code.SetDbValue(Rs, MenuItems.Code.CurrentValue, Null, MenuItems.Code.ReadOnly)

			' Field Name
			Call MenuItems.Name.SetDbValue(Rs, MenuItems.Name.CurrentValue, Null, MenuItems.Name.ReadOnly)

			' Field Description
			Call MenuItems.Description.SetDbValue(Rs, MenuItems.Description.CurrentValue, Null, MenuItems.Description.ReadOnly)

			' Field Vegetarian
			Call MenuItems.Vegetarian.SetDbValue(Rs, MenuItems.Vegetarian.CurrentValue, Null, MenuItems.Vegetarian.ReadOnly)

			' Field Spicyness
			Call MenuItems.Spicyness.SetDbValue(Rs, MenuItems.Spicyness.CurrentValue, Null, MenuItems.Spicyness.ReadOnly)

			' Field Price
			Call MenuItems.Price.SetDbValue(Rs, MenuItems.Price.CurrentValue, Null, MenuItems.Price.ReadOnly)

			' Field IdMenuCategory
			Call MenuItems.IdMenuCategory.SetDbValue(Rs, MenuItems.IdMenuCategory.CurrentValue, Null, MenuItems.IdMenuCategory.ReadOnly)

			' Field IdBusinessDetail
			Call MenuItems.IdBusinessDetail.SetDbValue(Rs, MenuItems.IdBusinessDetail.CurrentValue, Null, MenuItems.IdBusinessDetail.ReadOnly)

			' Field Photo
			Call MenuItems.Photo.SetDbValue(Rs, MenuItems.Photo.CurrentValue, Null, MenuItems.Photo.ReadOnly)

			' Field allowtoppings
			Call MenuItems.allowtoppings.SetDbValue(Rs, MenuItems.allowtoppings.CurrentValue, Null, MenuItems.allowtoppings.ReadOnly)

			' Field dishpropertygroupid
			Call MenuItems.dishpropertygroupid.SetDbValue(Rs, MenuItems.dishpropertygroupid.CurrentValue, Null, MenuItems.dishpropertygroupid.ReadOnly)

			' Field hidedish
			boolwrk = MenuItems.hidedish.CurrentValue
			If boolwrk&"" <> "1" And boolwrk&"" <> "0" Then boolwrk = ew_IIf(boolwrk&"" <> "", "1", "0")
			Call MenuItems.hidedish.SetDbValue(Rs, boolwrk, Null, MenuItems.hidedish.ReadOnly)

			' Field PrintingName
			Call MenuItems.PrintingName.SetDbValue(Rs, MenuItems.PrintingName.CurrentValue, Null, MenuItems.PrintingName.ReadOnly)

			' Field i_displaySort
			Call MenuItems.i_displaySort.SetDbValue(Rs, MenuItems.i_displaySort.CurrentValue, Null, MenuItems.i_displaySort.ReadOnly)
