...
' ASPMaker configuration for Table timezones
Dim timezones
...
Class ctimezones
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "offset", offset)
		Call ew_SetArObj(Fields, "offsetdst", offsetdst)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "timezones_Highlight"
	End Property
	...
	Public Property Get SqlSelect()
		SqlSelect = ew_IIf(zSqlSelect & "" <> "", zSqlSelect, "SELECT * FROM [dbo].[timezones]")
	End Property
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "timezoneslist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "timezoneslist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("timezonesview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("timezonesview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
	...
	Public Function AddUrl(parm)
		If parm <> "" Then
			AddUrl = "timezonesadd.asp?" & UrlParm(parm)
		Else
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("timezonesedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("timezonesadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("timezonesdelete.asp", UrlParm(""))
	End Function
	...
	Public Sub LoadListRowValues(RsRow)
		...
		offset.DbValue = RsRow("offset")
		offsetdst.DbValue = RsRow("offsetdst")
	End Sub
	...
	Sub RenderListRow()
		...
		' offset
		offset.ViewValue = offset.CurrentValue
		offset.ViewCustomAttributes = ""

		' offsetdst
		offsetdst.ViewValue = offsetdst.CurrentValue
		offsetdst.ViewCustomAttributes = ""
		...
		' offset
		offset.LinkCustomAttributes = ""
		offset.HrefValue = ""
		offset.TooltipValue = ""

		' offsetdst
		offsetdst.LinkCustomAttributes = ""
		offsetdst.HrefValue = ""
		offsetdst.TooltipValue = ""
	...
	Public Sub RenderEditRow()
		...
		' offset
		offset.EditAttrs.UpdateAttribute "class", "form-control"
		offset.EditCustomAttributes = ""
		offset.EditValue = offset.CurrentValue
		offset.PlaceHolder = ew_HtmlEncode(ew_RemoveHtml(offset.FldCaption))

		' offsetdst
		offsetdst.EditAttrs.UpdateAttribute "class", "form-control"
		offsetdst.EditCustomAttributes = ""
		offsetdst.EditValue = offsetdst.CurrentValue
		offsetdst.PlaceHolder = ew_HtmlEncode(ew_RemoveHtml(offsetdst.FldCaption))
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("offset", offset.ExportValue(Export))
					Call XmlDoc.AddField("offsetdst", offsetdst.ExportValue(Export))
				Else
					Call XmlDoc.AddField("ID", ID.ExportValue(Export))
					Call XmlDoc.AddField("Timezone", Timezone.ExportValue(Export))
					Call XmlDoc.AddField("offset", offset.ExportValue(Export))
		...
		If Not Doc.ExportCustom Then
			...
			If Doc.Horizontal Then ' Horizontal format, write header
				...
				If ExportPageType = "view" Then
					...
					If offset.Exportable Then Call Doc.ExportCaption(offset)
					If offsetdst.Exportable Then Call Doc.ExportCaption(offsetdst)
				Else
					If ID.Exportable Then Call Doc.ExportCaption(ID)
					If Timezone.Exportable Then Call Doc.ExportCaption(Timezone)
					If offset.Exportable Then Call Doc.ExportCaption(offset)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If Not Doc.ExportCustom Then
					...
					If ExportPageType = "view" Then
						...
						If offset.Exportable Then Call Doc.ExportField(offset)
						If offsetdst.Exportable Then Call Doc.ExportField(offsetdst)
					Else
						If ID.Exportable Then Call Doc.ExportField(ID)
						If Timezone.Exportable Then Call Doc.ExportField(Timezone)
						If offset.Exportable Then Call Doc.ExportField(offset)
...
'			End If
	...
	Public Property Get ID()
		If Not IsObject(m_ID) Then
			Set m_ID = NewFldObj("timezones", "timezones", "x_ID", "ID", "[ID]", "CAST([ID] AS NVARCHAR)", 3, 0, "[ID]", False, False, FALSE, "FORMATTED TEXT")
	...
	Public Property Get Timezone()
		If Not IsObject(m_Timezone) Then
			Set m_Timezone = NewFldObj("timezones", "timezones", "x_Timezone", "Timezone", "[Timezone]", "[Timezone]", 202, 0, "[Timezone]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get offset()
		If Not IsObject(m_offset) Then
			Set m_offset = NewFldObj("timezones", "timezones", "x_offset", "offset", "[offset]", "[offset]", 202, 0, "[offset]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set offset = m_offset
	End Property
	...
	Public Property Get offsetdst()
		If Not IsObject(m_offsetdst) Then
			Set m_offsetdst = NewFldObj("timezones", "timezones", "x_offsetdst", "offsetdst", "[offsetdst]", "[offsetdst]", 202, 0, "[offsetdst]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set offsetdst = m_offsetdst
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		...
		If IsObject(m_offset) Then Set m_offset = Nothing
		If IsObject(m_offsetdst) Then Set m_offsetdst = Nothing
