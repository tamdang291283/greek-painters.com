...
' Define page object
Dim timezones_add
Set timezones_add = New ctimezones_add
Set Page = timezones_add

' Page init processing
timezones_add.Page_Init()

' Page main processing
timezones_add.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Render event
timezones_add.Page_Render()
%>
<!--#include file="header.asp"-->
<script type="text/javascript">
// Page object
var timezones_add = new ew_Page("timezones_add");
timezones_add.PageID = "add"; // Page ID
var EW_PAGE_ID = timezones_add.PageID; // For backward compatibility
// Form object
var ftimezonesadd = new ew_Form("ftimezonesadd");
// Validate form
ftimezonesadd.Validate = function() {
...
ftimezonesadd.Form_CustomValidate = 
...
ftimezonesadd.ValidateRequired = true; // Use JavaScript validation
<% Else %>
ftimezonesadd.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% timezones_add.ShowPageHeader() %>
<% timezones_add.ShowMessage %>
<form name="ftimezonesadd" id="ftimezonesadd" class="form-horizontal ewForm ewAddForm" action="<%= ew_CurrentPage() %>" method="post">
<% If timezones_add.CheckToken Then %>
<input type="hidden" name="<%= EW_TOKEN_NAME %>" value="<%= timezones_add.Token %>">
<% End If %>
...
Class ctimezones_add
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			timezones.CurrentAction = ObjForm.GetValue("a_add") ' Get form action
		...
		Else
			...
			If Request.QueryString("ID").Count > 0 Then
				...
				Call timezones.SetKey("ID", timezones.ID.CurrentValue) ' Set up key
			Else
				Call timezones.SetKey("ID", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
				timezones.CurrentAction = "C" ' Copy Record
			Else
				timezones.CurrentAction = "I" ' Display Blank Record
		...
		Select Case timezones.CurrentAction
			...
			Case "A" ' Add new record
				timezones.SendEmail = True ' Send email on add success
				If AddRow(OldRecordset) Then ' Add successful
					...
					If ew_GetPageName(sReturnUrl) = "timezonesview.asp" Then sReturnUrl = timezones.ViewUrl("") ' View paging, return to view page with keyurl directly
		...
		timezones.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		...
		timezones.Timezone.OldValue = timezones.Timezone.CurrentValue
		timezones.offset.CurrentValue = Null
		timezones.offset.OldValue = timezones.offset.CurrentValue
		timezones.offsetdst.CurrentValue = Null
		timezones.offsetdst.OldValue = timezones.offsetdst.CurrentValue
	End Function
	...
	Sub RenderRow()
		...
		ElseIf timezones.RowType = EW_ROWTYPE_ADD Then ' Add row
	...
	Function AddRow(RsOld)
		...
		' Field Timezone
		Call timezones.Timezone.SetDbValue(Rs, timezones.Timezone.CurrentValue, Null, False)

		' Field offset
		Call timezones.offset.SetDbValue(Rs, timezones.offset.CurrentValue, Null, False)

		' Field offsetdst
		Call timezones.offsetdst.SetDbValue(Rs, timezones.offsetdst.CurrentValue, Null, False)
		...
		bInsertRow = timezones.Row_Inserting(RsOld, Rs)
		If bInsertRow Then
		...
		Set Rs = Nothing
		If AddRow Then
			timezones.ID.DbValue = RsNew("ID")
		End If
		If AddRow Then

			' Call Row Inserted event
			Call timezones.Row_Inserted(RsOld, RsNew)
		End If
	...
	Sub SetupBreadcrumb()
		...
		PageId = ew_IIf(timezones.CurrentAction = "C", "Copy", "Add")
