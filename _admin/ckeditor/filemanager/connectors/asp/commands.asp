...
Sub FileUpload( resourceType, currentFolder, sCommand, CKEcallback ) ' ***
	Dim oUploader
	...
	Else
		...
		if not (oFSO.FolderExists( sServerDir ) ) then
			...
			Dim sItemKey  ' ***
			If oUploader.File.Exists("NewFile") Then ' ***
				sItemKey	= "NewFile" ' ***
			ElseIf oUploader.File.Exists("upload") Then ' ***
				sItemKey	= "upload" ' ***
			Else ' ***
				Response.Write("No upload file") ' ***
				Response.End ' ***
			End If ' ***
			sFileName	= oUploader.File(sItemKey).Name ' ***
			sExtension	= oUploader.File(sItemKey).Ext ' ***
			...
			Do While ( True )
				...
				Else
					...
					oUploader.SaveAs sItemKey, sFilePath ' ***
	...
	Else
		...
		If CKEcallback = "" Then ' ***
			SendUploadResults sErrorNumber, "", "", "" ' ***
		Else ' ***
			SendCKEditorResults sErrorNumber, CKEcallback, "", "", "" ' ***
		End If ' ***
	End If
End Sub
